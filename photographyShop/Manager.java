...
public class Manager 
{
    ...
    private ArrayList<Photographer> employee;
    ...
    public Manager()
    {
        ...
        employee = new ArrayList<Photographer>();
    ...
    public void hire(String name)
    {
        ...
        employee.add(new Photographer(name));
    ...
    public void giveOutAssignments()
    {
        ...
        //take entire list of assignments and sort by priority, print out prioritized list
        Collections.sort(assignment);
       for(Assignment str: assignment)
       {
            System.out.println(str);
        }
        double maxWidth =0.0;
        System.out.println("number of assignments: "+assignment.size());
        System.out.println("number of photogs: "+employee.size());
        int i = 0;
        
        for (Photographer phtg:employee)
          {
              if (assignment.size()>0){
              portfolio.add(new Portfolio(phtg.takePicture(assignment.get(0).picDescription), phtg.getName()));
            System.out.println(phtg.getName()+" took picture "+assignment.get(0).picDescription);
            assignment.remove(0);//remove top assignment from assignment list
//             i++;

          }  
        }
//           while (=employee.size() && i<employee.size());
}
     ...
     */
    public void newAssignment(int priority, String picDescription)
    {
        // YOUR CODE HERE
        // How will you keep track of the unfinished assignments?
        assignment.add(new Assignment(priority, picDescription));
          System.out.println("added assignment "+picDescription);
    }
    ...
    public void checkPortfolio()
    {
        ...
        for (Portfolio pic:portfolio)
        {
            
           Picture drawnPic =new Picture(pic.getPhotoFile());
            System.out.println("total portfolio should be: "+ pic.getPhotoFile());
            drawnPic.translate(maxWidth,0.0);
            drawnPic.draw();
            int yPosName = drawnPic.getMaxY();
            Text name = new Text (maxWidth, yPosName, pic.getPhotogName());
            name.draw();
             maxWidth += drawnPic.getWidth()+10.0;
        }
